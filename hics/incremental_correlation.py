...
class IncrementalCorrelation:
  ...
  def _update_relevancy_table(self, new_relevancies):
    ...
    current_relevancies = current_relevancies.append(pd.DataFrame(data=0, index=new_index,
                                                                  columns=current_relevancies.columns))

    cur_iteration, cur_relevancy = current_relevancies.iteration, current_relevancies.relevancy
    new_iteration, new_relevancy = new_relevancies.iteration, new_relevancies.relevancy
    current_relevancies.loc[new_relevancies.index, 'relevancy'] = \
        (cur_iteration / (cur_iteration + new_iteration)) * cur_relevancy + \
        (new_iteration / (cur_iteration + new_iteration)) * new_relevancy

    current_relevancies.loc[new_relevancies.index, 'iteration'] += new_iteration
  ...
  def _update_redundancy_table(self, new_weights, new_redundancies):
    ...
    current_redundancies = (current_weights / (new_weights + current_weights)) * current_redundancies + \
        (new_weights / (new_weights + current_weights)) * new_redundancies
  ...
  def _add_slices_to_dict(self, subspace, slices, slices_store):
    ...
    if subspace_tuple not in slices_store:
      categorical = [{'name': ft, 'values': self.subspace_contrast.get_values(ft)}
