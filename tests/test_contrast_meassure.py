...
from rar.hics.contrast_meassure import HiCS
import numpy as np
import pandas as pd
...
class Test_HiCS(TestCase):
  ...
  def test_create_continuous_condition(self):
    ...
    chosen_indices = dataset.loc[np.logical_and(dataset['feature'] >= condition['from_value'],
                                                dataset['feature'] <= condition['to_value']), :].index.values
